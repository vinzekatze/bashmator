...
Каждый скрипт вносится в YAML файл. В этом файле помощью ключей задаются аргументы командной строки, параметры подстановки их значений в код скрипта, используемая оболочка, а также информация, по которой этот скрипт можно будет найти.
...
Если `default` - список из одного пустого значение, то аргумент будет опцией с пустым значением по умолчанию. Пример:
...
Содержит описание назначения аргумента, которая будет выведена при вызове помощи `use <script name> -h` или `use <script name> --help`. Пример:
...
Содержит имена множественных аргументов (`multiply: true`) в качестве ключей и строки-разделители для их объединения в качестве значений. По умолчанию объединение значений множественных аргументов происходит через пробел.
...
Таким образом, значения множественного аргумента `arg1` будут подставлены в скрипт в виде следующей строки:
...
Обычно этот ключ применяется для правильной подстановки неоднозначных значений с использованием шаблона `{0!r}`, однако в комбинации с пустыми по умолчанию аргументами может позволить вставлять в скрипт дополнительные куски при необходимости.
...
Содержит кратное описание файла, которое будет выведено при вызове помощи `use <script name> -h` или `use <script name> --help`. Пример:
...
Работает аналогично ключу `mode` в корне. Используется для выборочного изменения параметров форматирования из основного скрипта. 
...
Содержит кратное описание назначения `item`, которое будет выведено при вызове помощи `use <script name> -h` или `use <script name> --help`.
